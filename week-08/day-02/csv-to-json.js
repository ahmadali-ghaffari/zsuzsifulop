...
namelist = []
var myMap = lines.map(function(item){
    var oneRegistery = item.split(";")
    if (!namelist.includes(oneRegistery[0])){
        namelist.push(oneRegistery[0])
        return {name: oneRegistery[0], id: oneRegistery[1], color_code: oneRegistery[2]};
    }
});
var myMapFilter = myMap.filter(function(item){
     return item != undefined;
});
console.log(myMapFilter);
